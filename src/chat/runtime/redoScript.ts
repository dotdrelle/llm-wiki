...
async function redoMessage(btn) {
  ...
  // The click must answer immediately: the truncate round-trip takes a while
  // and a silent button read as broken, so mark it busy before the fetch and
  // restore it on any outcome.
  const prevLabel=btn.dataset.state||'';
