...
.act-body.activity-list-mode{overflow:hidden}.activity-subtabs{display:flex;flex-wrap:wrap;gap:4px;flex:none;margin-bottom:8px}.activity-subtab{flex:1 1 calc(50% - 2px);min-width:0;border:1px solid var(--border);border-radius:6px;background:var(--panel-soft);color:var(--muted);font:800 9px var(--font-sans);padding:5px 3px;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-transform:uppercase}.activity-subtab:hover,.activity-subtab.active{border-color:var(--accent);color:var(--accent);background:var(--accent-soft)}.activity-subtab:last-child{flex:1 1 100%}.activity-subtab.has-error{border-color:color-mix(in srgb,var(--err) 55%,var(--border));color:var(--err);background:color-mix(in srgb,var(--err) 10%,transparent)}.activity-subtab.has-running{border-color:color-mix(in srgb,var(--accent) 55%,var(--border));color:var(--accent);background:color-mix(in srgb,var(--accent) 12%,transparent)}.activity-subtab.writing{border-color:var(--accent);color:var(--accent);background:var(--accent-soft);animation:actWritingPulse 1.1s ease-in-out infinite}@keyframes actWritingPulse{0%,100%{box-shadow:0 0 0 0 transparent}50%{box-shadow:0 0 0 2px color-mix(in srgb,var(--accent) 35%,transparent)}}.activity-subtab-content{flex:1;min-height:0;overflow-y:auto;overscroll-behavior:contain;padding-right:2px}.activity-subtab-content>.act-card+.act-card{margin-top:6px}.activity-subtab-logs{display:flex;flex-direction:column;overflow:hidden}.activity-subtab-logs .runtime-log-filters{flex:none}.activity-subtab-logs .runtime-log{flex:1;min-height:0;max-height:none}.activity-subtab-logs .runtime-journal{max-height:none;flex:1;min-height:0}
...
/* A <canvas> without a sizing rule keeps its 300×150 intrinsic size, and the
   scene, a plain flex item without flex:1, settled for that size: the graph
   sat in a square stuck to the top-left of the frame. The wiki graph already
   had the right rules (.graph-explorer-canvas); this one did not. */
...
.bubble pre{background:var(--panel-deep);border-radius:8px;padding:10px 12px;margin:.5em 0;max-width:100%;overflow-x:auto;white-space:pre-wrap;overflow-wrap:anywhere}
...
/* Progress notes are Donna reporting on the turn in progress, not the answer:
   ...
   One feed, not one message per line: the last four entries stay visible
...
.runtime-progress-feed .feed-list{max-height:calc(4 * 1.6em + 10px);overflow-y:auto;font-size:12px;line-height:1.6;color:var(--muted);padding:5px 10px;border-top:1px solid var(--border);border-bottom:1px solid var(--border);scrollbar-width:thin}
