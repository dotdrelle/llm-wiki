...
async function fetchOllamaModelInfo(
  ...
  try {
    ...
    const arch = (modelInfo['general.architecture'] as string | undefined) ?? details.family ?? '';
...
function estimateKvCache(info: OllamaModelInfo, numCtx: number, kvCacheType: string): number {
  ...
  return Math.round(2 * numCtx * info.blockCount * info.kvHeadCount * headDim * bytesPerElement);
}
...
async function checkProvider(
  config: AppConfig,
): Promise<{ ollamaInfo?: OllamaModelInfo; effectiveNumCtx?: number; effectiveNumCtxSource?: string }> {
  ...
  try {
    ...
    err(`Ollama not reachable at ${baseUrl}: ${e instanceof Error ? e.message : String(e)}`);
    return {};
  }
  ...
  // Effective numCtx priority: .wikirc.yaml (sent per-request) > OLLAMA_CONTEXT_LENGTH
  // (server env, read from current process as best-effort) > modelfile > model native
  const envCtxVal = process.env.OLLAMA_CONTEXT_LENGTH
    ? parseInt(process.env.OLLAMA_CONTEXT_LENGTH, 10)
    : undefined;
  const effectiveNumCtx =
    config.llm.numCtx ?? envCtxVal ?? ollamaInfo.modelfileNumCtx
...
function printOllamaHardware(
  ...
  suggestions: Record<string, Record<string, string | number>>,
): void {
  console.log('\n── System & model ───────────────────────────────────────────');
  ...
  const paramStr =
    info.parameterCount > 0
      ? `${(info.parameterCount / 1e9).toFixed(1)}B`
      : '(unknown)';
  ...
  const envFlash = process.env.OLLAMA_FLASH_ATTENTION;
  const envKvType = (process.env.OLLAMA_KV_CACHE_TYPE ?? 'f16').toLowerCase();
  row('OLLAMA_CONTEXT_LENGTH:', envCtx ?? '(not set — Ollama default)');
  row('OLLAMA_FLASH_ATTENTION:', envFlash ?? '(not set)');
  row('OLLAMA_KV_CACHE_TYPE:', envKvType);

  const displayNumCtx = effectiveNumCtx ?? 2048;
  row('Effective numCtx:', `${displayNumCtx.toLocaleString()} tokens (from ${effectiveNumCtxSource})`);
  ...
  if (kvCacheBytes > 0) {
    row('Estimated KV cache:', `${gb(kvCacheBytes)} (${envKvType}, ctx=${displayNumCtx.toLocaleString()})`);
  }
  if (totalEstimate > 0) {
  ...
  if (!config.llm.numCtx) {
    ...
    ok(`numCtx ${config.llm.numCtx?.toLocaleString()} (model native max: ${info.nativeCtx > 0 ? info.nativeCtx.toLocaleString() : '?'})`);
  }

  if (!envFlash) {
    warn('OLLAMA_FLASH_ATTENTION not set — recommended for faster inference: launchctl setenv OLLAMA_FLASH_ATTENTION 1');
  } else if (envFlash === '1') {
  ...
  if (envKvType === 'f16') {
    warn(
      'OLLAMA_KV_CACHE_TYPE=f16 (default) — q8_0 halves KV cache memory with minimal quality loss',
    );
    if (kvCacheBytes > 0) {
      const saved = kvCacheBytes / 2;
      warn(`  → switching to q8_0 would save ~${gb(saved)} of KV cache`);
    }
  } else {
...
export default async function doctorCmd(config: AppConfig): Promise<void> {
  ...
  console.log('\n── Provider ────────────────────────────────────────────────');
  const { ollamaInfo, effectiveNumCtx, effectiveNumCtxSource } = await checkProvider(config);

  const suggestions: Record<string, Record<string, string | number>> = {};

  if (config.llm.provider === 'ollama' && ollamaInfo) {
    printOllamaHardware(config, ollamaInfo, effectiveNumCtx, effectiveNumCtxSource ?? 'Ollama default', suggestions);
  }
  ...
  row('wiki pages:', `${pages.length} (avg ${avg(pageSizes)} chars, max ${pageSizes.length > 0 ? Math.max(...pageSizes) : 0} chars)`);
  row('chunks:', `${allChunks.length} (avg ${avg(chunkSizes)} chars, p95 ${pct(chunkSizes, 95)} chars, max ${chunkSizes.length > 0 ? Math.max(...chunkSizes) : 0} chars)`);
  row('index.md:', `${indexContent.length} chars`);
  row('ingested (archive):', `${rawIngestedPages.length} file(s) in raw/ingested`);
  row('pending ingest:', `${untrackedPaths.length} file(s) in raw/untracked${untrackedSizes.length > 0 ? ` (avg ${avg(untrackedSizes)} chars, max ${maxUntracked} chars)` : ''}`);

  const largeUntracked = untrackedContents.filter((u) => u.size > config.retrieval.maxSourceChars);
  ...
  if (truncatedChunks.length > 0) {
    warn(`${truncatedChunks.length}/${allChunks.length} chunks exceed maxChunkChars and will be truncated`);
  }
  ...
  if (config.llm.temperature > 0.15) {
    warn(`temperature ${config.llm.temperature} → 0.1 recommended for structured JSON tasks`);
  ...
  if (Math.abs(recommendedChunkChars - config.retrieval.maxChunkChars) > 500) {
    warn(`maxChunkChars ${config.retrieval.maxChunkChars} → ${recommendedChunkChars} (p95 chunk: ${p95chunk} chars)`);
  ...
  const recommendedSourceChars =
    untrackedSizes.length > 0
      ? Math.min(Math.max(maxUntracked + 500, config.retrieval.maxSourceChars), Math.floor(available * 0.15))
      : config.retrieval.maxSourceChars;
  if (maxUntracked > config.retrieval.maxSourceChars) {
    warn(`maxSourceChars ${config.retrieval.maxSourceChars} → ${recommendedSourceChars} (largest pending source: ${maxUntracked} chars)`);
    (suggestions.retrieval ??= {}).maxSourceChars = recommendedSourceChars;
    hasWarnings = true;
  } else if (untrackedSizes.length > 0) {
    ok(`maxSourceChars ${config.retrieval.maxSourceChars} (largest pending source: ${maxUntracked} chars)`);
  } else {
    ok(`maxSourceChars ${config.retrieval.maxSourceChars} (no pending sources in raw/untracked)`);
  }
  ...
  if (currentPerSlotUsage > perSlotBudget) {
    err(`maxContextFiles ${config.retrieval.maxContextFiles} × maxChunkChars ${config.retrieval.maxChunkChars} = ${currentPerSlotUsage} chars/slot exceeds per-slot budget (~${perSlotBudget} chars)`);
    ...
    warn(`maxContextFiles ${config.retrieval.maxContextFiles} → ${recommendedMaxContextFiles} (budget allows, avg chunk ${effectiveChunkSize} chars)`);
