...
.community-document-index{flex:none;display:flex;max-height:260px;min-height:0;flex-direction:column;padding:10px;background:#08131ef2;border:1px solid var(--community-color,var(--line));border-radius:7px;box-shadow:0 10px 28px #0008}
.community-document-index h4{flex:none;margin:0 0 8px;color:var(--text)}.community-document-index-list{min-height:0;overflow:auto;overscroll-behavior:contain}
.community-document-index button{display:grid;width:100%;grid-template-columns:28px minmax(0,1fr);gap:7px;align-items:start;border:0;background:transparent;padding:5px;text-align:left;color:var(--muted)}.community-document-index button:hover,.community-document-index button.selected{background:var(--soft);color:var(--text)}
