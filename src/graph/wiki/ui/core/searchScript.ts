...
function activateSearch(id){const input=document.querySelector('#search'),result=id?data.nodes.find(n=>n.id===id):searchMatches(input.value)[0];if(!result)return;input.value=result.title;document.querySelector('#graph-search-results').hidden=true;selectedCommunity=data.communities.find(c=>c.nodeIds.includes(result.id))?.id||null;view='focus';selectDocument(result);render()}
`;
}
