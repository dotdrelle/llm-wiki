...
function updateCommunityFilterCounts(){
  ...
  document.querySelectorAll('.community-group').forEach(group=>{
    ...
    const members=community?community.nodeIds.filter(kept):[];
...
*/
function visible(){const enabled=new Set([...document.querySelectorAll('[data-type]:checked')].map(x=>x.dataset.type));let nodes=data.nodes.filter(n=>enabled.has(n.type));if(selected&&view==='focus'){const base=new Set([selected.id]),scope=new Set(base);data.edges.forEach(e=>{if(base.has(e.from))scope.add(e.to);if(base.has(e.to))scope.add(e.from)});nodes=nodes.filter(n=>scope.has(n.id))}else if(selectedCommunity&&view!=='map'){const community=data.communities.find(c=>c.id===selectedCommunity),base=new Set(community?.nodeIds||[]),scope=new Set(base);data.edges.forEach(e=>{if(base.has(e.from))scope.add(e.to);if(base.has(e.to))scope.add(e.from)});nodes=nodes.filter(n=>scope.has(n.id))}const ids=new Set(nodes.map(n=>n.id));return{nodes:nodes.map(n=>({...n})),edges:data.edges.filter(e=>ids.has(e.from)&&ids.has(e.to)).map(e=>({...e}))}}
`;
}
