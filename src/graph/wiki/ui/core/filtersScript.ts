...
function communityDocsHtml(community){
  ...
  return community.nodeIds.filter(id=>enabled.has(typeById.get(id))).map(id=>{const n=data.nodes.find(x=>x.id===id);return '<div class="community-doc-row"><button class="community-doc" data-doc="'+esc(id)+'" title="'+esc(id)+'">'+esc(graphLeafDisplay(n?.title||id))+'</button><button type="button" class="community-doc-action" data-preview-doc="'+esc(id)+'" title="Preview">'+graphIcon('preview')+'</button><button type="button" class="community-doc-action" data-send-doc="'+esc(id)+'" title="Add to Donna" aria-label="Add '+esc(n?.title||id)+' to Donna">'+graphIcon('donna')+'</button></div>'}).join('')}
