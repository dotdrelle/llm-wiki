...
1. If the CME connector (cme_status) is available, call it. Report whether it is configured or not.
2. If the production connector is available, call `production_list_jobs` to list recent jobs. Report running jobs, pending jobs, and any recent failures.
